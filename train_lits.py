...
from monai.transforms import RandGaussianNoised, RandGibbsNoised, RandGaussianSmoothd, RandFlipd, RandAffined, NormalizeIntensityd, ToTensord, Compose, \
...
from utils.FistulaDataLoader import Union_Dataloader
...
def get_dataloaders(args):
    train_transforms = Compose(
        [
            ...
            RandGibbsNoised(keys='image', prob=0.1),
            RandGaussianSmoothd(keys='image', prob=0.1),

            RandFlipd(keys=['image', 'label'], prob=0.1),
            RandAffined(
                keys=['image', 'label'],
                prob=0.1,
    ...
    train_dataset = LiTS2017(base_dir="../medical_data/LiTS/processed/train", transform=train_transforms)
    val_dataset = LiTS2017(base_dir="../medical_data/LiTS/processed/val", transform=val_transforms)
...
class BaseTrainer:
    ...
    def get_dice_score(self, prev_masks, gt3D):
        def compute_dice(mask_pred, mask_gt):
            ...
            if volume_sum == 0:
                # return np.NaN
                return torch.tensor(0.0, device=mask_gt.device)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs):
            ...
            if not self.args.multi_gpu or (self.args.multi_gpu and self.args.rank == 0):
                ...
                logger.info(
                    f'Epoch\t {epoch}\t : loss: {epoch_loss}, dice: {epoch_dice}, val_loss: {val_loss}, val_dice: {val_dice}')
