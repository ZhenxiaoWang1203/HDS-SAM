...
from utils.FistulaDataLoader import FistulaDataSet

parser = argparse.ArgumentParser()
parser.add_argument('-tdp', '--test_data_path', type=str, default='../medical_data/esophageal/fistula')
...
if __name__ == "__main__":
    ...
    fistula_train, fistula_test = read_csv('../medical_data/esophageal/esophageal_fistula.csv')
    test_dataset = FistulaDataSet(data=fistula_test, transform=test_transforms,
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        voxel = (batch_data['image_segment'].to(device=device).to(torch.float32) == 1).sum().item()
        sz = image3D.size()
        ...
        pred_path = os.path.join(vis_root, os.path.basename(img_name[0]).replace(".npy", f"_pred{args.num_clicks - 1}.npy"))
