...
from monai.transforms import Compose, NormalizeIntensityd, ToTensord, ScaleIntensityRanged, ScaleIntensityd

from utils.LiTSDataLoader import LiTS2017
...
import json
import pickle
from utils.click_method import get_next_click3D_torch_ritm, get_next_click3D_torch_2, get_next_click3D_torch

parser = argparse.ArgumentParser()
parser.add_argument('-tdp', '--test_data_path', type=str, default='../medical_data/LiTS/processed/test')
parser.add_argument('-vp', '--vis_path', type=str, default='./visualization')
parser.add_argument('-cp', '--checkpoint_path', type=str, default='./work_dir/union_train/sam_model_dice_best.pth')
...
parser.add_argument('-pm', '--point_method', type=str, default='ritm')
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    ...
    for num_click in range(num_clicks):
        with torch.no_grad():
            ...
            iou_list.append(round(compute_iou(medsam_seg, gt3D[0][0].detach().cpu().numpy()), 4))
            dice_list.append(round(compute_dice(gt3D[0][0].detach().cpu().numpy().astype(np.uint8), medsam_seg), 4))
            hd = float(torch.mean(compute_hausdorff_distance(torch.from_numpy(medsam_seg).unsqueeze(0).unsqueeze(0), gt3D)))
            hd_list.append(hd)

    return pred_list, click_points, click_labels, iou_list, dice_list, hd_list


if __name__ == "__main__":
    test_transforms = Compose(
        [
            ScaleIntensityRanged(keys=['image'], a_min=0, a_max=0.7, b_min=0, b_max=1, clip=True),
            NormalizeIntensityd(keys=['image'], nonzero=False, allow_missing_keys=True),
            ScaleIntensityd(keys=['image'], minv=0.0, maxv=1.0, allow_missing_keys=True),
            ToTensord(keys=['image', 'label'])
        ]
    )

    test_dataset = LiTS2017(base_dir="../medical_data/LiTS/processed/test", transform=test_transforms)
    ...
    for batch_data in tqdm(test_dataloader):
        image3D, gt3D, img_name = batch_data['image_patch'], batch_data['image_segment'], batch_data['image_path']
        voxel = batch_data['tumor_volume']
        sz = image3D.size()
        if(sz[2]<args.crop_size or sz[3]<args.crop_size or sz[4]<args.crop_size):
            print("[ERROR] wrong size", sz, "for", img_name)
        vis_root = './pred/union_train'
        pred_path = os.path.join(vis_root, os.path.basename(img_name[0]), "_pred.npy")

        if(args.skip_existing_pred and os.path.exists(pred_path)):
            iou_list, dice_list = [], []
            hd_list = []
            for iter in range(args.num_clicks):
                curr_pred_path = os.path.join(vis_root, os.path.basename(img_name[0]).replace(".npy", f"_pred{iter}.npy"))
            ...
            if(args.dim==3):
                seg_mask_list, points, labels, iou_list, dice_list, hd_list = finetune_model_predict3D(
            ...
            print("save to", os.path.join(vis_root, os.path.basename(img_name[0]).replace(".npy", "_pred.npy")))
            pt_path=os.path.join(vis_root, os.path.basename(img_name[0]).replace(".npy", "_pt.pkl"))
            pickle.dump(pt_info, open(pt_path, "wb"))
            for idx, pred3D in enumerate(seg_mask_list):
                out = sitk.GetImageFromArray(pred3D)
                sitk.WriteImage(out, os.path.join(vis_root, os.path.basename(img_name[0]).replace(".npy", f"_pred{idx}.nii")))

        per_iou = max(iou_list)
        all_iou_list.append(per_iou)
        all_dice_list.append(max(dice_list))
        all_hd_list.append(min(hd_list))
        if voxel < 6000:
            all_dice_list_small.append(max(dice_list))
            all_hd_list_small.append(min(hd_list))
        else:
            all_dice_list_large.append(max(dice_list))
            all_hd_list_large.append(min(hd_list))
        print(dice_list)
        print(hd_list)
    ...
    print('Mean HD: ', sum(all_hd_list) / (len(all_hd_list)))
    print('Mean Dice Small: ', sum(all_dice_list_small) / len(all_dice_list_small))
    print('Mean HD Small: ', sum(all_hd_list_small) / (len(all_hd_list_small)))
    print('Mean Dice Large: ', sum(all_dice_list_large) / len(all_dice_list_large))
    print('Mean HD Large: ', sum(all_hd_list_large) / (len(all_hd_list_large)))
    ...
    with open(args.save_name, 'w') as f:
        ...
        f.writelines(f'# mean hd: \t{np.mean(all_hd_list)}\n')
        f.writelines(f'# mean dice small: \t{np.mean(all_dice_list_small)}\n')
        f.writelines(f'# mean hd small: \t{np.mean(all_hd_list_small)}\n')
        f.writelines(f'# mean dice large: \t{np.mean(all_dice_list_large)}\n')
        f.writelines(f'# mean hd large: \t{np.mean(all_hd_list_large)}\n')
